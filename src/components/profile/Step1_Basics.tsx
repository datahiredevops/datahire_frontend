...
export default function Step1_Basics({ data, update, onNext }: Step1Props) {
  ...
  const [phoneCountryCode, setPhoneCountryCode] = useState("US"); // Track ISO Code (US, CA)
  const [dialCode, setDialCode] = useState(COUNTRY_CODES[0].dial_code);
  ...
  useEffect(() => {
    ...
    // Smartly parse the phone number
    if (data.phone) {
        // Find best match (longest dial code first to distinguish +1 and +1242)
        ...
        if (match) {
            // Check if we have a stored country preference to disambiguate US vs Canada
            // If the dial code matches the "Country" field, prefer that one.
            const countryMatch = COUNTRY_CODES.find(c => c.name === data.country && c.dial_code === match.dial_code);
            
            if (countryMatch) {
                setPhoneCountryCode(countryMatch.code);
                setDialCode(countryMatch.dial_code);
            } else {
                // Default to the match found
  ...
  // --- PHONE LOGIC ---
  const handlePhoneCountrySelect = (country: typeof COUNTRY_CODES[0]) => {
    setPhoneCountryCode(country.code); // Store exact country (US vs CA)
  ...
  const selectedPhoneCountry = COUNTRY_CODES.find(c => c.code === phoneCountryCode) || COUNTRY_CODES[0];
  ...
  // --- LOCATION COUNTRY LOGIC ---
  const handleLocationCountrySelect = (country: typeof COUNTRY_CODES[0]) => {
    setFormData(prev => ({ ...prev, country: country.name }));
    update({ ...data, country: country.name });
    setIsCountryDropdownOpen(false);
    setCountrySearchQuery("");

    // OPTIONAL: Auto-sync Phone Code if user changes Location Country
    // Only if phone number is empty (don't overwrite existing user data)
    if (!phoneNumber) {
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
      ...
      <form onSubmit={handleSave} className="space-y-6">
                        ...
                        <div className="absolute z-50 top-full left-0 mt-1 w-72 bg-white border border-slate-200 rounded-xl shadow-xl max-h-80 flex flex-col animate-in fade-in zoom-in-95 duration-100">
                            <div className="p-2 border-b border-slate-100 sticky top-0 bg-white rounded-t-xl">
                                <div className="relative">
                                    ...
                                    <input type="text" autoFocus placeholder="Search country..." value={phoneSearchQuery} onChange={(e) => setPhoneSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-[#0F172A] outline-none" />
                                </div>
                            </div>
                            <div className="overflow-y-auto flex-1 p-1">
                                {filteredPhoneCountries.map((country) => (
                                    <button key={country.code} type="button" onClick={() => handlePhoneCountrySelect(country)} className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg hover:bg-slate-50 transition ${country.code === phoneCountryCode ? "bg-slate-100 font-medium text-slate-900" : "text-slate-600"}`}>
                                        <div className="flex items-center gap-3"><span className="text-lg">{country.flag}</span><span>{country.name}</span></div>
                                        <span className="text-slate-400 text-xs font-mono">{country.dial_code}</span>
                                    </button>
                                ))}
                                {filteredPhoneCountries.length === 0 && <div className="p-4 text-center text-slate-400 text-xs">No countries found</div>}
                            </div>
                        </div>
                    )}
                </div>
        ...
        {/* Location (Searchable Country + City Input) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative" ref={countryDropdownRef}>
            ...
            <button type="button" onClick={() => setIsCountryDropdownOpen(!isCountryDropdownOpen)} className="mt-1 w-full bg-white border border-slate-300 rounded-lg px-4 py-2 text-left flex justify-between items-center focus:ring-2 focus:ring-[#0F172A] focus:border-[#0F172A]">
                <span className={formData.country ? "text-slate-900" : "text-slate-400"}>{formData.country || "Select Country"}</span>
            ...
            {isCountryDropdownOpen && (
                <div className="absolute z-50 top-full left-0 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-xl max-h-60 flex flex-col animate-in fade-in zoom-in-95 duration-100">
                    <div className="p-2 border-b border-slate-100 sticky top-0 bg-white rounded-t-xl">
                        <div className="relative">
                            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                            <input type="text" autoFocus placeholder="Search..." value={countrySearchQuery} onChange={(e) => setCountrySearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-[#0F172A] outline-none" />
                        </div>
                    </div>
                    <div className="overflow-y-auto flex-1 p-1">
                        {filteredLocationCountries.map((c) => (
                            <button key={c.code} type="button" onClick={() => handleLocationCountrySelect(c)} className={`w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-slate-50 transition ${formData.country === c.name ? "bg-slate-100 font-medium text-slate-900" : "text-slate-600"}`}>
                                <span className="mr-2">{c.flag}</span> {c.name}
                            </button>
                        ))}
                        {filteredLocationCountries.length === 0 && <div className="p-4 text-center text-slate-400 text-xs">No countries found</div>}
                    </div>
                </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="url" name="linkedin_url" className="mt-1 block w-full rounded-lg border border-slate-300 px-4 py-2 focus:ring-[#0F172A] outline-none" placeholder="https://linkedin.com/in/..." value={formData.linkedin_url} onChange={handleChange} />
            </div>
            <div>
                <label className="block text-sm font-medium text-slate-700">Portfolio URL</label>
                <input type="url" name="portfolio_url" className="mt-1 block w-full rounded-lg border border-slate-300 px-4 py-2 focus:ring-[#0F172A] outline-none" placeholder="https://myportfolio.com" value={formData.portfolio_url} onChange={handleChange} />
            </div>
        </div>
